...
function deleteMyPanorama($usr,$pID){
    ...
    $str = substr($imgURL, 39);//取網址由前算起27字元之後(http://localhost/backendPHP)的全部字元
    $filename = '.'.$str; //  . + /PanoramaImg/heiufuiwef.jpg
    if(file_exists($filename)){
        unlink($filename);//刪除全景圖文件
        if(($smallimgURL !== "")&&(isset($smallimgURL))){//如果縮圖不為空
            $str = substr($smallimgURL, 39);//取網址由前算起27字元之後(http://localhost/backendPHP)的全部字元
            $filename = '.'.$str;
            if(file_exists($filename)){
...
function deleteMyPanoramaFile($pID,$usr){
    ...
    if(isset($rs["imgLink"]) && $rs["imgLink"] !== ""){
        ...
        $str = substr($OriginimgURL, 39);//取網址由前算起27字元之後(http://localhost/backendPHP)的全部字元
        $filename = '.'.$str;
        if(file_exists($filename)){
            unlink($filename);//刪除底部圖文件
        }
...
function deleteMySmallPanoFile($pID,$usr){
    ...
    if(isset($rs["smallimgLink"]) && $rs["smallimgLink"] !== ""){
        ...
        $str = substr($OriginSmallimgURL, 39);//取網址由前算起27字元之後(http://localhost/backendPHP)的全部字元
        $filename = '.'.$str;
        if(file_exists($filename)){
            unlink($filename);//刪除底部圖文件
        }
...
function DelInfoPic($usr){
    global $db;
       ...
       if($rs = mysqli_fetch_assoc($result)){ 
           $imgLink = $rs["photo"];
           $d=substr($imgLink, 40,7);
           $file=substr($imgLink, 48);
           $fileName=substr($imgLink, 40);
    ...
    if($rs["total"] !== 0 ){ //userID有沒有該展場(eID)
        ...
        while($rs2 = mysqli_fetch_assoc($result)){
            ...
            if(($musicLink !== "")&&(isset($musicLink))){//如果語音不為空
                $str = substr($musicLink, 39);//取網址由前算起27字元之後(http://localhost/backendPHP)的全部字元
                $filename = '.'.$str;
                if(file_exists($filename)){
                ...
                if($rs3 = mysqli_fetch_assoc($result3)){
                    if(isset($rs3['smallimgLink'])){
                        ...
                        if($smallimgLink !== $thumbnailLink){//不是引用原先的panorama的縮圖
                            $str = substr($thumbnailLink, 39);//取網址由前算起27字元之後(http://localhost/backendPHP)的全部字元
                            $filename = '.'.$str;
                            if(file_exists($filename)){
                                unlink($filename);//刪除文件
                            }      
                ...
                if((isset($rs3['imageLink'])) && ($rs3['imageLink'] !== "")){
                    ...
                    if((isset($rs3['2DimgLink'])) && ($rs3['2DimgLink'] !== "")){
                        ...
                        if($imageLink !== $img2DLink){//不是引用原先的panorama的縮圖
                            $str = substr($imageLink, 39);//取網址由前算起27字元之後(http://localhost/backendPHP)的全部字元
                            $filename = '.'.$str;
                            if(file_exists($filename)){
                                unlink($filename);//刪除文件
                            }      
                ...
                if((isset($rs3['cMusicLink'])) && ($rs3['cMusicLink'] !== "")){
                    ...
                    if((isset($rs3['iMusicLink'])) && ($rs3['iMusicLink'] !== "")){
                        ...
                        if($cMusicLink !== $iMusicLink){//不是引用原先的panorama的縮圖
                            $str = substr($cMusicLink, 39);//取網址由前算起27字元之後(http://localhost/backendPHP)的全部字元
                            $filename = '.'.$str;
                            if(file_exists($filename)){
                                unlink($filename);//刪除文件
                            }      
        ...
        $str = substr($frontPicture, 39);//取網址由前算起27字元之後(http://localhost/backendPHP)的全部字元
        $filename = '.'.$str;
        if(file_exists($filename)){
            unlink($filename);//刪除文件
            if(($mapImg !== "")&&(isset($mapImg))){//如果縮圖不為空
                $str = substr($mapImg, 39);//取網址由前算起27字元之後(http://localhost/backendPHP)的全部字元
                $filename = '.'.$str;
                if(file_exists($filename)){
                    unlink($filename);//刪除文件
                }
            ...
            if(($picture2 !== "")&&(isset($picture2))){//如果縮圖不為空
                $str = substr($picture2, 39);//取網址由前算起27字元之後(http://localhost/backendPHP)的全部字元
                $filename = '.'.$str;
                if(file_exists($filename)){
                    unlink($filename);//刪除文件
                }
            ...
            if(($picture3 !== "")&&(isset($picture3))){//如果縮圖不為空
                $str = substr($picture3, 39);//取網址由前算起27字元之後(http://localhost/backendPHP)的全部字元
                $filename = '.'.$str;
                if(file_exists($filename)){
                    unlink($filename);//刪除文件
                }
...
function DelItem3D($iID){
    ...
    if($rs = mysqli_fetch_assoc($result)){ 
        ...
        $objd=substr($objectLink, 40,6);//28
        $objfile=substr($objectLink, 47);//35
        $objfileName=substr($objectLink, 40);//28
...
function DelItemMusic($iID){
    ...
    if($rs = mysqli_fetch_assoc($result)){ 
        $musicLink = $rs["musicLink"];
        if($musicLink!=""){
            $md=substr($musicLink, 40,9);//28
            $mfile=substr($musicLink, 50);//38
            $mfileName=substr($musicLink, 40);//28
...
function DeleteItem($usr,$iID){
    ...
    if($rs = mysqli_fetch_assoc($result)){ 
        ...
        $objectLink = $rs["3DobjectLink"];
        $musicLink = $rs["musicLink"];
        $imgd=substr($imgLink, 40,7);
        $imgfile=substr($imgLink, 48);
        $imgfileName=substr($imgLink, 40);
        ...
        //3DobjectLink
        $objd=substr($objectLink, 40,6); //return "Item3D"
        $objfile=substr($objectLink, 47); // return fileName
        $objfileName=substr($objectLink, 40); // return Item3D/fileName
        if($objod=opendir($objd)){ //$d是目錄名
            while(($objf=readdir($objod))!==false){ //讀取目錄內檔案
                if($objf===$objfile){
                    unlink($objfileName);
        ...
        //musicLink
        if($musicLink!=""){
           $md=substr($musicLink, 40,9);
           $mfile=substr($musicLink, 50);
           $mfileName=substr($musicLink, 40);
            if($mod=opendir($md)){ //$d是目錄名
                while(($mf=readdir($mod))!==false){ //讀取目錄內檔案
                    if($mf===$mfile){
                        unlink($mfileName);
