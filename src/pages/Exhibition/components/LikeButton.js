...
const LikeButton = (props) => {
  ...
  useEffect(() => {
    axios({ // getExhibitionLikes(eID) // 抓like的數量
        method: "get",
        url: 'http://360.systemdynamics.tw/backendPHP/Control.php?act=getExhibitionLikes&eID='+ eID,
        dataType: "JSON",
    ...
    .then((res) => {
        ...
        if(res.data.Login){ // 使用者有登入
            axios({ // getLikeorNot(eID) 檢查使用者是否已經按下該展場的like button
                method: "get",
                url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=getLikeorNot&eID="+eID,
                dataType: "JSON",
                withCredentials: true
            })
            .then((res) => {
  ...
  const AddLike = async(eID) => {
    axios({
        method: "get",
        url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=AddLike&eID="+eID,
        dataType: "JSON",
        withCredentials: true
    })
    .then((res) => {
  ...
  const CancelLike = async(eID) => { 
    axios({
        method: "get",
        url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=CancelLike&eID="+eID,
        dataType: "JSON",
        withCredentials: true
    })
    .then((res) => {
