...
const Item = () => {
    ...
    useEffect(() => {
        axios({
            method: "post",
            url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=getMyItemStatistics",
            dataType: "JSON",
    ...
    useEffect(() => {
        axios({
            method: "get",
            url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=getItem",
            dataType: "JSON",
            withCredentials: true
        })
            .then((res) => {
                                ...
                                <tbody>
                                    {
                                        show && (
                                            searchTerm.slice(indexOfFirstPost, indexOfLastPost).map((item) => {
                                                return (
                                                    <ItemTableData key={item.iID} item={item} />
                                                )
                                            })
                                        )
                                    }
