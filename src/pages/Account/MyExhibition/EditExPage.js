...
const EditExPage = () => {
    ...
    axios({
      ...
      .then((res) => {
        if (res.data.isYourEx) {
          axios({
            method: "get",
            url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=getMyExPanoData&eID=" + ary2[1],
            withCredentials: true,
  ...
  const editMyExhibition = (e) => { //當按下完成後將表單所有資料送入後端
    ...
    if ((data.exhibitionName !== "") && (data.eIntro !== "") && (data.startTime !== "") && (data.closeTime !== "")
      ...
      axios({
        method: "post",
        url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=editMyExhibition",
        data: sendData,
        withCredentials: true,
        headers: { "Content-Type": "multipart/form-data" }
      })
