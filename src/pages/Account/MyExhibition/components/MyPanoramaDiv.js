...
import InfoSpotModal from './PreviewPanorama/InfoSpotModal';
import ExhibitionMap from "./PreviewPanorama/ExhibitionMap";
import "./PreviewPanorama/testExStyles.css";
import { Button, Row, Col } from "react-bootstrap";

const MyPanoramaDiv = (props) => {
    const { eID } = props; //eID
    const { eIDisYours } = props;
    ...
    useEffect(() => {
        if (firstSceneisLoad && mapIsLoad) {
            document.getElementById("testRightPanel").onclick = hideMenu;
            var sMenu = document.getElementById("testSlidingMenu");
            sMenu.style.left = "0px";
            var panelIcon = document.getElementById("testRightPanel");
    ...
    useEffect(() => {
        if (firstSceneisLoad) {
            if (pMusic !== "") {
                document.getElementById("testPMusicUpPanel").onclick = showMusicMenu;
                var sMenu = document.getElementById("testPMusicControls");
                sMenu.style.bottom = "-50px";
                var panelIcon = document.getElementById("testPMusicUpPanel");
        ...
        function showMusicMenu() {
            ...
            if (pos === 0) {
                setMusicMenuIcon("▼");
                document.getElementById("testPMusicUpPanel").onclick = hideMusicMenu;
            }
        }
    }, [firstSceneisLoad, pMusic]);

    useEffect(() => { //第一次跑useEffect是在畫面剛加載時，當時props都還沒有傳eID參數進來
        if ((eID > 0) && (eIDisYours === true) && (firstSceneisLoad === false)) { //因為上一個檔案預設如果還沒接到eID就是0，所以我們就寫一個if擋掉eID還沒定義時的情況
            axios({
                method: "get",
                url: 'http://360.systemdynamics.tw/backendPHP/Control.php?act=getFirstPanoramaData&eID=' + eID,
                dataType: "JSON",
                ...
                .then((res) => {
                    ...
                    if ((res.data.mapImg !== "") && (res.data.mapImg !== null)) {
                        ...
                        axios({
                            method: "get",
                            url: 'http://360.systemdynamics.tw/backendPHP/Control.php?act=getAllPanoramaXY&eID=' + eID,
                            dataType: "JSON",
                            withCredentials: true
                        })
                            .then((res) => {
    ...
    }, [eID, eIDisYours, firstSceneisLoad]);//eID如果變了，會在觸發一次useEffect
    ...
    useEffect(() => {
        ...
        function addPanorama(sceneId) {
            ...
            axios({
                method: "get",
                url: 'http://360.systemdynamics.tw/backendPHP/Control.php?act=getOtherPanoramaData&epID=' + sID,
                dataType: "JSON",
                withCredentials: true
            })
                .then((res) => {
    ...
    function setCustomSpotUI(hotSpotDiv, args) {
        ...
        span.style.marginLeft = -(span.scrollWidth - hotSpotDiv.offsetWidth) / 2 - elem.scrollWidth + 'px';
                    ...
                    && (
                        <>
                            <ReactPannellum id="panorama" imageSource={image} sceneId={sceneId} style={style} config={config}>
                                {(mapImgLink !== "") && (
                                    <div id="controls">
                                        <div id="testSlidingMenu" className="testSlidingMenu">
                                            <div id="testmenu" className="testmenu" ref={vmenuRef}>
                                                <span style={{ color: 'blue', fontWeight: 'bold' }}>展場平面地圖</span>
                                                <br />
                                ...
                                {(pMusic !== "") && (
                                    <div id="testPMusicControls">
                                        <div id="testPMusicSlidingMenu" className="testPMusicSlidingMenu">
                                            <div id="testPMusicUpPanel">
                                                <span id="musicMenuIcon" >{musicMenuIcon}</span>
                                            </div>
                                            <div id="testPMusicVmenu" className="testPMusicVmenu" ref={exVmenuRef}>
                                                <span id="testPMusicName" style={{ color: 'black', fontWeight: 'bold' }}>{exhibitionName}</span>
                                                <br />
                            ...
                            <Row className="me-0">
                                <Col className='d-flex justify-content-end'>
                                    <Link to={"/myExhibition"} className='d-flex justify-content-center' style={{width:"125px"}}>
                                        <Button className="finish_btn">
                                            回到我的展場
                                        </Button>
                                    </Link>
                                </Col>
                                <Col>
                                    <Link to={"/myDetailExhibition?eID=" + eID} className='d-flex justify-content-center' style={{width:"125px"}}>
