...
const EditExhitbitiveExPage = () => {
  ...
  useEffect(() => {
    ...
    axios({
      method: "get",
      url: 'http://360.systemdynamics.tw/backendPHP/Control.php?act=checkIsYourEx&eID=' + ary2[1],
      dataType: "JSON",
      ...
      .then((res) => {
        if (res.data.isYourEx) {
          axios({
            method: "get",
            url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=getMyExData&eID=" + ary2[1],
            withCredentials: true,
  ...
  const editMyExhibition = (e) => { //當按下完成後將表單所有資料送入後端
    ...
    if ((data.eID !== -1) && (data.exhibitionName !== "") && (data.eIntro !== "") && (data.startTime !== "")
      ...
      axios({
        method: "post",
        url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=editMyExData&eID=" + data.eID,
        data: sendData,
        withCredentials: true,
        headers: { "Content-Type": "multipart/form-data" }
      })
