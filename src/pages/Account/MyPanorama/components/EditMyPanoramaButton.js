...
const EditMyPanoramaButton = (props) => {
        ...
        axios({ // isLogin() // 先判斷是否登入，才能去紀錄使用者按下喜歡行為
            ...
            .then((res) => {
                if (res.data.Login) { // 有登入
                    axios({ // 撈目前這個pID資料出來
                        method: "get",
                        url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=checkPanoramaAccess&pID=" + pID,
                        dataType: "JSON",
                        withCredentials: true
                    })
                        .then((res) => {
    ...
    const submitForm = (e) => {
        ...
        if (info.panoramaName !== "") {
            ...
            axios({
                method: "post",
                url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=updateMyPanorama&pID=" + pID,
                dataType: "JSON",
                data: sendData,
                withCredentials: true
            })
