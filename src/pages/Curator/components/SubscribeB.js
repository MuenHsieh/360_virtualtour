...
const SubscribeB = (props) => {
  ...
  useEffect(() => {
    axios({
        method: "get",
        url: 'http://360.systemdynamics.tw/backendPHP/Control.php?act=getCuratorSubs&id='+ id,
        dataType: "JSON",
    ...
    .then((res) => {
        if(res.data.Login){ //使用者有登入
            axios({ //getLikeorNot(eID)
                method: "get",
                url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=SubscribeOrNot&id=" + id,
                dataType: "JSON",
                withCredentials: true
            })
            .then((res) => {
  ...
  const subscribe = async(id) => {
    axios({
        method: "get",
        url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=subscribe&id="+id,
        dataType: "JSON",
        withCredentials: true
    })
    .then((res) => {
  ...
  const unSubscribe = async(id) => { 
    axios({
        method: "get",
        url: "http://360.systemdynamics.tw/backendPHP/Control.php?act=unSubscribe&id="+id,
        dataType: "JSON",
        withCredentials: true
    })
    .then((res) => {
